...
CREATE INDEX IF NOT EXISTS idx_messenger_messages_mentions ON messenger_messages USING GIN(mentions) WHERE mentions IS NOT NULL;
...
-- Story indexes
CREATE INDEX IF NOT EXISTS idx_messenger_stories_user_active ON messenger_stories(user_id) WHERE expires_at > NOW();

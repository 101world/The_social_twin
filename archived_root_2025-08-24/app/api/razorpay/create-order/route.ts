...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.RAZORPAY_KEY_SECRET || !(process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || process.env.RAZORPAY_KEY_ID)) {
    ...
    const order = await razorpay.orders.create({
      ...
      currency,
  receipt: `rc_${Date.now().toString(36)}_${(userId || '').slice(-4)}`.slice(0, 40),
      notes: {
        userId,
